    ...
    if  datetime.now().minute == 1 and datetime.now().hour > 3 and datetime.now().hour < 19:
        ...
        try:
            response = requests.get(
                'https://api.openweathermap.org/data/2.5/onecall?lat=47.549761&lon=33.911244&exclude=minutely,daily&appid=189ca286824a5c3da649d8b954beca4c')
            data = json.loads(response.text)
            forecast_data = data["hourly"][3]
...
def WriteNewLinesToFile(forecast_data):
    ...
    with open('data/forecasted_data_OpenWeather.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([forecast_data["dt"],forecast_data["dt_string"],round(forecast_data["temp"]-273.15,2),forecast_data["clouds"],forecast_data["visibility"]])
