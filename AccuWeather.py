    ...
    if  datetime.now().minute == 1 and datetime.now().hour > 3 and datetime.now().hour < 19:
        ...
        try:
            ...
            response = requests.get(
                'http://dataservice.accuweather.com/forecasts/v1/hourly/12hour/322548?apikey=I5Z5fGppFX66hGm1UaK272tI9nPos39R&language=en-us&details=true&metric=true', params=PARAMS)
            data = json.loads(response.text)
            forecast_data = data[2]
            forecast_data['EpochDateTime'] = forecast_data['EpochDateTime'] + 10800
            forecast_data['dt_string'] = datetime.utcfromtimestamp(forecast_data['EpochDateTime']).strftime('%Y-%m-%d %H:%M:%S')
            print(forecast_data)
...
def WriteNewLinesToFile(forecast_data):
    ...
    with open('data/forecasted_data_AccuWeather.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([forecast_data["EpochDateTime"],forecast_data["dt_string"],forecast_data["Temperature"]['Value'],forecast_data["Visibility"]['Value'],forecast_data["Ceiling"]['Value'],forecast_data["CloudCover"]])
